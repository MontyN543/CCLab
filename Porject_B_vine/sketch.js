...
function setup() {
  ...
  mountainShape = [
    ...
    createVector(520, 180),
    createVector(640, 360)
  ];

  // Create 28 vines for a dense overgrowth
  for (let i = 0; i < 28; i++) {
    let baseX = map(i, 0, 27, 0, 640);
    let angle = baseX < 100 ? radians(30)
               : baseX > 520 ? radians(-30)
               : 0;
...
function draw() {
  // interpolate sky color toward target
  ...
  push();
    ...
    ocean.update(glacier.isReforming);
    ocean.draw();
  ...
  // ── Year Target Logic ──
  if (sequenceComplete || icebergActive || cityActive || mountainActive) {
    targetYear = 2025;
  } else {
    targetYear = 2050;
  }
...
function mousePressed() {
  ...
  if (x >  80 && x < 300 && y > 140 && y < 660) {
    buildingManager.reformOnce();
    cityActive = true;
    setTimeout(() => cityActive = false, 600);
  }

  // Mountain click
  if (x > 260 && x < 900 && y > 240 && y < 420) {
  ...
  if (!sequenceComplete) {
    if      (clickStage === 0 && x > 560 && x < 800 && y > 260 && y < 500) clickStage = 1;
    else if (clickStage === 1 && x >  80 && x < 300 && y > 140 && y < 660) clickStage = 2;
    else if (clickStage === 2 && x > 260 && x < 900 && y > 240 && y < 420) {
      clickStage = 3;
...
function generatePlants() {
  ...
  while (plants.length < 120) {
    let x = random(10, 630), y = random(180, 360);
...
class Glacier {
  ...
  update() {
    ...
    this.particles.forEach(p => {
      if (this.isReforming) {
        p.reform();
        if (!p.isFullyReformed()) fully = false;
      } else {
        p.melt();
      }
    });
...
class BuildingManager {
  constructor() {
    ...
    [
      ...
      {x:10,  y:120, w:35, h:260},
      {x:120, y:150, w:38, h:230},
      {x:160, y:130, w:30, h:250},
      {x:40,  y:280, w:30, h:120},
      {x:90,  y:290, w:25, h:110},
      {x:150, y:300, w:28, h:100},
      {x:70,  y:270, w:22, h:130},
      {x:0,   y:380, w:60, h:30},
      {x:80,  y:395, w:55, h:35},
      {x:160,y:400,w:50, h:25},
...
class Vine {
  constructor(baseX, angle) {
    ...
    this.greenShade = map(this.baseX, 0, 640, 100, 200);
    this.generatePath();
  }
  ...
  startRetract(state) {
    this.targetRetract = state ? 1 : 0;
  }

  update() {
    this.retractAmount = lerp(this.retractAmount, this.targetRetract, 0.05);
  }
